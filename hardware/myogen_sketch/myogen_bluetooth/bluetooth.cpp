...
// External variable for gyro streaming control (declared in main .ino file)
extern bool gyro_streaming_enabled;
...
bool blue_controller::get_servos(struct uHand_Servo* uhand_servos)
{
  ...
  else if(resule_oj.func == CMD_START_GYRO_STREAM)
  {
    gyro_streaming_enabled = true;
    Serial.println("📡 Gyro streaming started");
    return false; // Don't process as servo command
  }
  else if(resule_oj.func == CMD_STOP_GYRO_STREAM)
  {
    gyro_streaming_enabled = false;
    Serial.println("📡 Gyro streaming stopped");
    return false; // Don't process as servo command
  }
  else{
    Serial.print("func:");
